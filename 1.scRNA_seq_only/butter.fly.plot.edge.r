...
core_matrix = as.matrix(edge_tis@assays$RNA@data)
...
for(each in core_AUC_list){
    ...
    AUC <- data.frame(AUC)
    #print(AUC)
    edge_tis <- AddMetaData(edge_tis, metadata = AUC)
}

### Define D (aka y-axis)
core_meta <- edge_tis@meta.data
...
new2$sample<-c(rep("edge",4))
pdf("edge.barplot.pdf")
...
new4<-read.table("new.core.txt",header=T)
new3<-rbind(new2,new4)
new3
new3$Var1<-factor(new3$Var1,levels=c("NPC","OPC","AC","MES"))

pdf("core.edge.barplot.pdf")
ggplot(new3,aes(x=sample,y=Freq,fill=Var1,label=Freq))+scale_fill_manual(values=c("#0571B0","#92C5DE","#F4A582","#CA0020"))+geom_col(aes(fill = Var1 ), width = 0.7)+theme_classic()+theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))+scale_y_continuous(breaks = round(seq(0, 100, by = 20),1))+geom_text(size=4,colour="white",position = position_stack(vjust=0.5))
dev.off()
